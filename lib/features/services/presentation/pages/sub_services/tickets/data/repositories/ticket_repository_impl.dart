...
class TicketRepositoryImpl implements TicketRepository {
  ...
  Future<Either<Failure, TicketList>> getTicketList(String requestType) async {
    if (await networkInfo.isConnected) {
      try {
        final responseModels = await remoteDataSource.getTicketList(
          requestType,
        );
    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        final details = TicketDetails(
          ...
          documents:
              responseModel.documents.map((doc) {
                return TicketDocument(id: doc.id, url: doc.url, name: doc.name);
              }).toList(),
          activity:
              responseModel.activity.map((act) {
          ...
          assigneeTicket:
              responseModel.assigneeTicket != null
                  ? TicketUser(
                  ...
                  : null,
          createdByUser:
              responseModel.createdByUser != null
                  ? TicketUser(
                  ...
                  : null,
          ticketCategory:
              responseModel.ticketCategory != null
                  ? TicketCategory(
                    ...
                    hasSubCategories:
                        responseModel.ticketCategory!.hasSubCategories,
                  )
                  : null,
          ticketSubCategory: responseModel.ticketSubCategory,
          followers:
              responseModel.followers.map((follower) {
  ...
  Future<Either<Failure, List<UploadedFile>>> uploadTicketFile(
    ...
    if (await networkInfo.isConnected) {
      try {
        final responseModels = await remoteDataSource.uploadTicketFile(
          clientId,
          ticketId,
          filePath,
        );
  ...
  Future<Either<Failure, String>> deleteTicketFile(
    ...
    if (await networkInfo.isConnected) {
      try {
        final message = await remoteDataSource.deleteTicketFile(
          supportDocumentId,
          fileId,
        );
