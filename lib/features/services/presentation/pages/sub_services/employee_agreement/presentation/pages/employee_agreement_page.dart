...
class _EmployeeAgreementPageState extends State<EmployeeAgreementPage> {
            ...
            return BlocBuilder<AgreementBloc, AgreementState>(
              builder: (context, state) {
                if (state is AgreementLoading) {
                  return Center(child: _buildShimmerList(context));
                }
                ...
                if (state is AgreementLoaded) {
                  final agreements =
                      state.agreementList.agreements
                          .map(
                            (agreement) =>
                                AgreementMapper.toEmployeeAgreementModel(
                                  agreement,
                                ),
                          )
                          .toList();
  ...
  Widget _buildShimmerList(BuildContext context) {
    ...
    final spacing = size.height < 700 ? 12.0 : 16.0;
        ...
        return Padding(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                // 1. Title Shimmer
                AppShimmer.rectangular(height: 20, width: size.width * 0.6),
                SizedBox(height: size.height * 0.02),

                // 2. Detail Rows (Replicating _buildDetailRow)
                _buildShimmerDetailRow(context, hasAvatar: true),
                // Employee Name
                SizedBox(height: size.height * 0.015),
                _buildShimmerDetailRow(context),
                // Agreement Type
                SizedBox(height: size.height * 0.015),
                _buildShimmerDetailRow(context, hasAvatar: true),
                // Assigned By
                ...
                Row(
                  children: [
                    const AppShimmer.rectangular(height: 12, width: 50),
                    const SizedBox(width: 10),
                    AppShimmer.rectangular(height: 12, width: size.width * 0.2),
                  ],
                ),
              ],
            ),
    ...
    return Row(
      children: [
        ...
        SizedBox(
          ...
          child: const AppShimmer.rectangular(height: 12, width: 60),
        ),
        const Spacer(),
        // Avatar + Value Shimmer
        if (hasAvatar) ...[
          const AppShimmer.circular(width: 24, height: 24),
          const SizedBox(width: 8),
        ],
        AppShimmer.rectangular(height: 12, width: screenWidth * 0.2),
      ],
    );
  }
}
